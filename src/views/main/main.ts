...
export default class Main extends Vue {
  private normalModal: {
    option: { size: string[]; align: string[] };
    ui: UIOptionType;
    title: string;
    content: string;
  } = {
    ...
    this.$modal.on(
      {
        ...
        positiveMessage:
          this.normalModal.ui.positiveMessage !== ''
            ? this.normalModal.ui.positiveMessage
            : undefined,
        negativeMessage:
          this.normalModal.ui.negativeMessage !== ''
            ? this.normalModal.ui.negativeMessage
            : undefined,
      },
      ...
      await this.$promiseModal.on(
        {
          ...
          positiveMessage:
            this.promiseModal.ui.positiveMessage !== ''
              ? this.promiseModal.ui.positiveMessage
              : undefined,
          negativeMessage:
            this.promiseModal.ui.negativeMessage !== ''
              ? this.promiseModal.ui.negativeMessage
              : undefined,
        },
  ...
  private async onMultiplePromiseModal() {
    try {
      await this.$promiseModal.on(
        ...
        this.promiseModalMultiple.title !== ''
          ? this.promiseModalMultiple.title
          : undefined
      );
  ...
  private async onClickUITestMulti(
    option: UIOptionType,
    content: string,
    title: string,
    buttons: string[]
    ...
    try {
      console.log('promise modal on');
      const idx = await this.$multiPromiseModal.on(
        option,
        content,
        title,
        buttons
