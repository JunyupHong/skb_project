...
export default class Main extends Vue {
    ...
    private async onClickUITestMulti(option: UIOptionType, content: string, title: string, buttons: string[]) {
        try {
            console.log('promise modal on');
            const idx = await this.$multiPromiseModal.on(option, content, title, buttons);
            console.log('promise modal off - resolve', '\nreturn value: ', idx);
        } catch (e) {
            console.log('promise modal off - reject');
        }
